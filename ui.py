...
class QuizInterface:
    def __init__(self, quiz_brain: QuizBrain):
        ...
        self.question_text = self.canvas.create_text(
            150, 125, text="This is text", fill=THEME_COLOR, font=("arial", 20, "italic"), width=280)
        self.canvas.grid(row=1, column=0, columnspan=2, pady=50)

        # Buttons
        check_img = PhotoImage(file="./images/true.png")
        self.check = Button(
            image=check_img, highlightthickness=0, command=self.true)
        self.check.grid(row=2, column=0)

        x_img = PhotoImage(file="./images/false.png")
        self.x = Button(image=x_img, highlightthickness=0, command=self.false)
